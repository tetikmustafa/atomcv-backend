...
/**
 * Which way to set the keep-mark on a generation (Bolum 35.2).
 *
 * <p><strong>One endpoint both ways, as the support grant is</strong>
 * (Bolum 48.4.1). The resource map names {@code POST /archive} and no undo,
 * and a mark that cannot be taken off is a trap rather than a control — the
 * grant settled the same question the same way, with a boolean in the body of
 * the endpoint that set it.
 *
