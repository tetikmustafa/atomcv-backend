 ...
 *
 * <p><strong>What the mark is for, and what it is not yet.</strong> The schema
 * pairs the column with {@code pdf_expires_at}: an artifact is kept fourteen
 * days, and an archived one is kept for good. No artifact is stored today — a
 * download re-renders from {@code content_snapshot} and the {@code 410} path
 * arrives with R2 — so nothing expires in either direction and the mark
 * changes no retention. It is the owner's mark on their own history until
 * storage lands, and the row the retention rule will read on the day it does
 * (the open paragraph is the reminder).
 *
