 ...
 * An empty one is not an error: a provider without a key is skipped silently,
 * which is what lets one chain definition serve a deployment that has
 * configured one vendor and a deployment that has configured five.
 *
...
public record OpenRouterProperties(
    ...
    /**
     * OpenRouter has two ways to be held to a schema, with a fall back "if
     * unsupported". Which one a model supports is a fact about that model
     * rather than something the response reliably says, so it is configuration
     * rather than detection — guessing from an error string would silently
     * downgrade every failure into the weaker mode.
     */
