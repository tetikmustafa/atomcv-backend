...
public record QuotaSubject(Type type, String id) {
     ...
     *
     * <p><strong>The address and not the session</strong>, for the reason that
     * matters: a session is a cookie, and a cookie is something anybody can
     * throw away and ask for another. Counting by session would give an
     * unlimited allowance to whoever clears theirs, and the whole point of
     * these counters is that this product is free and the calls behind it are
     * not.
     *
     ...
     * is the one setting it ({@code forward-headers-strategy}).
     */
