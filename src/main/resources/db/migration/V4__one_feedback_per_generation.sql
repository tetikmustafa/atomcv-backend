-- generation_feedback carries no key beyond its id, so nothing
...
-- the generation handler refuses an ownerless job — and when anonymous
-- generation lands, its own subject will need its own answer here.
