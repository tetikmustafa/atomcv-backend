...
class ProfileHeadIT extends AbstractIntegrationTest {
    ...
    void resolvingTwiceReturnsTheSameProfile() {
        ...
        // Narrowed to this user, and it had to be: since anonymous profiles
        // became rows with an expiry rather than a Redis document, the shared
        // test database carries anonymous profiles belonging to nobody. A
        // global count now measures the rest of the suite instead of this
        // case.
        assertThat(jdbc.queryForObject("SELECT count(*) FROM profiles WHERE user_id = ?",
                Integer.class, user.userId())).isEqualTo(1);
    }
