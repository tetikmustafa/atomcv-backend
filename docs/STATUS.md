...
**2026-09-16** · **frontend'e on dört madde açık** — `B-100`…`B-113`, denetimlerden, hiçbiri henüz ACK'lenmedi
...
**Aşama 4 + beş denetim.** **Faz G** (`B-088`, `B-089`), **üç şablon** (`B-090`, `B-092`), **Katman B** (`B-091`), **başvuru takibi** (`B-093`), **DOCX** (`B-094`), **yaşam döngüsü e-postaları** (`B-096`). Sonra spec **beş kez** koda karşı denetlendi (09-15, 09-16 ×4); beşinin de bulduğu her şey ya yazıldı ya gerekçesiyle `spec/`'e kaydedildi. Anlatı `notes/`'ta ve `notes/archive/denetim-*.md`'de.
...
**Test:** 1907 birim · 580 entegrasyon · latex 145 — 0 hata; artı elle koşulan `embeddingTest` ve `llmEval` (para harcar)
...
**Sırada `B-100`…`B-113` var (2026-09-16).** On dördü de denetimlerden ve hiçbiri ACK'lenmedi, yani `to-frontend.md` 100 satırı geçti — bu bir arşivleme değil koordinasyon meselesi. **Önce `npm run gen:api`**: birinci turda altı uç ve üç şema, ikincide `SelectionLine`, beşincide iki sözlük (`B-112`). Başlıcanlar: `B-100` (CSP — kod işi yok ama dağıtımda görülmeli), `B-101` (`contract-check`'in URL'i), `B-103` (etiketler), `B-107` (`auto` ilanı takip ediyor: üretim yavaşlayabilir), `B-108` (`heldBackReason` dört ayrı cümle istiyor — `INACTIVE` profil ayarı, `EXCLUDED_BY_DIRECTIVE` bu CV'nin düzenlemesi), **`B-111`** (çeviri dosyalarınız yanlış tablodan yazılmış olabilir: 27 koda karşı 41), ve **`B-113`** (`EXTRACTION_TIMEOUT` artık telde — 503'ünkinden farklı bir cümle istiyor).
